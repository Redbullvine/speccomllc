-- Keep KMZ/site import role-free. Any signed-in user can import into the
-- selected project, and KMZ snapshots are not gated by legacy role checks.
    ...
    begin
      ...
      if v_items is not null and jsonb_typeof(v_items) = 'array' then
        ...
        select
          ...
          case
            when nullif(trim(coalesce(item->>'qty','')), '') ~ '^-?[0-9]+(\.[0-9]+)?$'
              then nullif(trim(coalesce(item->>'qty','')), '')::numeric
            else null
          end,
          v_user_id
...
do $$
begin
  if to_regclass('public.project_kmz_snapshots') is not null then
    execute 'alter table public.project_kmz_snapshots enable row level security';

    execute 'drop policy if exists "project_kmz_snapshots_read_members" on public.project_kmz_snapshots';
    execute 'drop policy if exists "project_kmz_snapshots_write_members" on public.project_kmz_snapshots';
    execute 'drop policy if exists "project_kmz_snapshots_update_members" on public.project_kmz_snapshots';
    execute 'drop policy if exists "project_kmz_snapshots_delete_members" on public.project_kmz_snapshots';
    execute 'drop policy if exists "project_kmz_snapshots_read_authenticated" on public.project_kmz_snapshots';
    execute 'drop policy if exists "project_kmz_snapshots_write_authenticated" on public.project_kmz_snapshots';
    execute 'drop policy if exists "project_kmz_snapshots_update_authenticated" on public.project_kmz_snapshots';
    execute 'drop policy if exists "project_kmz_snapshots_delete_authenticated" on public.project_kmz_snapshots';

    execute 'create policy "project_kmz_snapshots_read_authenticated" on public.project_kmz_snapshots for select to authenticated using (auth.uid() is not null)';
    execute 'create policy "project_kmz_snapshots_write_authenticated" on public.project_kmz_snapshots for insert to authenticated with check (auth.uid() is not null)';
    execute 'create policy "project_kmz_snapshots_update_authenticated" on public.project_kmz_snapshots for update to authenticated using (auth.uid() is not null) with check (auth.uid() is not null)';
    execute 'create policy "project_kmz_snapshots_delete_authenticated" on public.project_kmz_snapshots for delete to authenticated using (auth.uid() is not null)';
  end if;
end $$;
