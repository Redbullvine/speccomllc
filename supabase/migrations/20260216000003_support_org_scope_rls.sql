-- Harden SUPPORT to org scope. ROOT remains global bypass.
  ...
  exists (
    ...
    where p.id = auth.uid()
      and (
        ...
        or public.effective_role_code(p.role_code, p.role) = 'ROOT'
        or (
          public.effective_role_code(p.role_code, p.role) = 'SUPPORT'
  ...
  exists (
    ...
    where p.id = auth.uid()
      and (
        ...
        or (
          public.effective_role_code(p.role_code, p.role) = 'SUPPORT'
          and (p.org_id = billed_by_org_id or p.org_id = billed_to_org_id)
        )
      )
  )
);
  ...
  exists (
    ...
    where i.id = invoice_id
      and (
        ...
        or (
          public.effective_role_code(p.role_code, p.role) = 'SUPPORT'
          and (p.org_id = i.billed_by_org_id or p.org_id = i.billed_to_org_id)
        )
      )
  )
);
...
-- Messages: global notices are ROOT-only unless SUPPORT row is org-scoped.
...
-- Tiered invoice generation: SUPPORT org-scoped, ROOT global.
...
begin
  ...
  if v_role_code <> 'ROOT' and v_org_id <> p_from_org_id then
...
-- Project grant RPC: SUPPORT is org-scoped; ROOT remains global.
...
begin
  ...
  if v_role_code = 'SUPPORT' and (v_org_id is null or v_org_id <> v_project_org_id) then
